...
//    }
//
//    def makeReactionObject(zipFBCReaction: (org.sbml.jsbml.Reaction, FBCReactionPlugin)): Reaction = {
...
//    val reactionObjects = zipFBCReactions.map(makeReactionObject)
...
//    def filterOperator(operList: List[Association]): List[Association] = {
...
//        val polys = lgp.map(getOrCreateGeneProduct)
//        val enzyme = Enzyme(name = "Reaction name")
//        val enzymeNode = enzyme.upload(graphDataBaseConnection)
//        polys.foreach(_.createRelationshipTo(enzymeNode, BiomeDBRelations.isA))
//        List(enzymeNode)
//      case _ =>
//        logger.error("Not a GeneProductRef: " + listOfAssociations)
//        List()
...
//    else {
//      val createdGeneProduct = graphDataBaseConnection.createNode(
//        DynamicLabel.label("Polypeptide"), DynamicLabel.label("To_check")
//      )
...
////  }
//
//  def reactionLoop(fbcReaction: FBCReactionPlugin): List[Object] = {//
